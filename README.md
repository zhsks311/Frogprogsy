...
3. Save an API key, use OAuth for supported providers such as Codex/ChatGPT, xAI, and Kimi, or import Kiro through the official `kiro-cli` with `frogp login kiro`. For Anthropic Claude, keep subscription auth in Claude Code homes; adding the Anthropic provider creates a forward-auth model-picker entry without storing a Claude token in frogprogsy.
