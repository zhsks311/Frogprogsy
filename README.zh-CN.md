...
已配置的模型停止提供或暂时失败时，可以在 dashboard **Models** 页面或 `frogp models continuity` 中查看受影响的设置和精确修复操作。自动替代在你显式启用前保持关闭，也不会自动改写已选择的模型。完整说明见 [frogp 命令](https://zhsks311.github.io/frog-progsy/zh-cn/reference/cli/#models)、[配置参考](https://zhsks311.github.io/frog-progsy/zh-cn/reference/configuration/#模型连续性)与[仪表盘流程](https://zhsks311.github.io/frog-progsy/zh-cn/guides/web-dashboard/#替换已停止提供的模型)。
