...
| OAuth | Login/status/logout for account-backed providers (`codex`, `xai`, `kimi`, `kiro`); login completion and logout both refresh the readiness-filtered routed catalog/cache. `GET /api/oauth/providers` returns each provider's login owner: browser-owned flows use the dashboard start/poll API, while Kiro is terminal-owned and the dashboard shows `frogp login kiro` instead of fabricating a browser flow. A dashboard login click explicitly replaces an unfinished browser attempt, aborts its poller, and returns a fresh auth URL/device code; an abandoned in-memory flow also expires after the dashboard's five-minute polling window. Codex device-code creation retries transient network failures three times but does not retry rejected 4xx requests. Kiro status means a FrogProgsy credential copy is stored, not that a billable upstream probe ran; its connection test reports `probe_unavailable`. Kiro logout removes only FrogProgsy's copy and never logs out or modifies the native Kiro CLI store. Native Anthropic subscription login is intentionally absent from this OAuth store. A dedicated Claude grant is a separate opt-in credential family owned by the grant broker, not an OAuth-provider row. |
