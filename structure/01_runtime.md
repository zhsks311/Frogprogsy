...
`frogp refresh` probes a bounded set of concrete addresses for a configured hostname. Accepting a current
management endpoint or replacing a stale runtime requires its health response, recorded PID, and listener
metadata to agree on the same concrete endpoint; legacy health without `processPid` still requires that proof. IPv4 and IPv6
