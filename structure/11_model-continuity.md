...
Reference-bound policy edits, replacements, and removals compare the complete normalized policy observed by the caller, not only the selected candidate. Indexed replacement and removal compare both the resolved target and the opaque raw-list revision. After any model lookup yields, the server synchronously re-reads the current policy and raw owner before validating and mutating. It therefore rejects concurrent writes after a sibling, order, raw alias, owner metadata, or policy change, even when the owner that moved into a stale index resolves to the same provider/model. It then uses owner-specific validation, config save, and catalog refresh paths; it never performs arbitrary JSON-path mutation.

Owner semantics constrain changes. A provider's `defaultModel` may change only to another model at that same configured provider and cannot be removed; changing the global default provider remains the existing provider-management action. Owners that store both provider and model may replace both fields. A gateway alias has no mutable config owner and never becomes required work merely because its historical target retired. Persisted aliases for configured providers remain inactive diagnostics even when their route is no longer in the current model list.

Model-mixing applicability follows the targets its runtime can actually select. Rules mode keeps only reachable rules plus its first configured roster fallback; the `count_tokens` path keeps the first configured roster target active even when an explicit pipeline or panel handles message requests. Fusion panel applicability follows the eight-member cap and, when multiround is enabled, the initial `budgetCalls - 1` panel-call cap. Saved but unreachable rows remain diagnostics.

The dashboard provides one section on the existing Models page. Required work is grouped by target and shows honest model and location counts. Each problem states the exact owner role/index, current target, reason, and a change or removal action. Eligible ordinary route targets also expose the ordered fallback sequence and automatic mode. Diagnostic and ready references remain collapsed. Successful continuity, visibility, and subagent-priority saves reload the report without coupling those settings together. A successful continuity change reconciles only its exact subagent owner when the authoritative saved order matches the predicted result. The subagent-order GET returns an opaque whole-list revision; dashboard saves send it back, so a direct external edit or a second edit after reconciliation returns `409` instead of being overwritten. A stale or unexpectedly changed saved order preserves the visible priority draft and blocks further priority/indexed writes until the user explicitly loads the latest saved order. A fallback-policy change preserves a distinct unsaved policy draft and blocks retry until the user explicitly accepts the latest saved policy. Responses from explicit reload/discard requests cannot overwrite a priority edit begun while that request was pending. Visibility changes never mutate priority order. A saved priority name that is absent from available candidates remains visible as an unavailable saved item instead of being silently removed.
