...
- Model continuity warnings now distinguish active settings that require action from inactive and past-session diagnostics, group duplicate targets with honest model/location counts, attribute faulty saved fallbacks to the candidate and exact policy position instead of a healthy primary, follow the effective helper and model-mixing runtime targets, retain old configured-provider aliases as diagnostics, and support exact optional-setting removal. Policy and indexed edits now reject concurrent policy, sibling, order, raw-alias, or metadata changes, including overlapping duplicate-owner requests. The Models page preserves separate unsaved fallback-policy and subagent-priority drafts, uses coherent conditional priority snapshots and writes, blocks stale retries until explicit review, keeps unavailable saved names visible without offering them as candidates, and omits targets that the current continuity report identifies as retired from replacement choices.
