...
export function classifyRawClaudePath(path: string, realPath: string, launcherBinDir: string, fs: FileSystemProbe = defaultFs): RawClaudeKind {
  ...
  // A removed legacy package bin can survive as ~/.bun/bin/claude -> node_modules/frogprogsy/src/claude.ts.
  // Classify by package ownership before the generic executable-basename fallback, or doctor would call it native.
  if (haystack.includes(`${sep}node_modules${sep}frogprogsy${sep}`)) return "managed_launcher";
  ...
  try {
    ...
    if (script.includes("Generated by frogprogsy") || script.includes("runClaudeLauncherProcess")) return "managed_launcher";
...
function rawClaudeReason(kind: RawClaudeKind): string {
  switch (kind) {
    case "managed_launcher": return "frogprogsy가 소유한 평범한 claude 명령이 PATH에서 먼저 발견됩니다. 이 이름은 원래 Claude Code의 것이어야 합니다.";
    case "package_bin": return "패키지 매니저가 설치한 Claude Code 바이너리가 PATH에서 먼저 발견됩니다.";
    case "cmux_shim": return "cmux shim이 PATH에서 먼저 발견됩니다.";
    case "native": return "원래 Claude 실행 파일이 PATH에서 먼저 발견됩니다.";
...
export function buildFindings(report: ClaudeDoctorReport): ClaudeDoctorFinding[] {
  ...
  if (report.rawClaude.kind === "managed_launcher") {
    findings.push({
      severity: "error",
      code: "raw_claude_hijacked_by_frogprogsy",
      message: "평범한 `claude` 명령이 frogprogsy 바로가기나 이전 package bin에 가로채여 원래 Claude Code가 실행되지 않습니다.",
      action: "현재 개발 설치를 remove→reinstall한 뒤 `frogp refresh`로 계정별 바로가기를 다시 만드세요. 확인: `which -a claude`의 첫 항목이 frogprogsy 관리 디렉터리나 frogprogsy package를 가리키면 안 됩니다.",
    });
  } else if (report.rawClaude.kind === "cmux_shim") {
    findings.push({ severity: "warning", code: "raw_claude_cmux_shim", message: "현재 PATH의 claude는 cmux shim입니다.", action: "cmux 밖의 새 터미널에서 `which -a claude`를 확인하세요. 계정을 명시적으로 고를 때는 claude-work/claude-personal 같은 계정별 바로가기를 사용하세요." });
  }
  const missingOnPath = report.launchers.filter(launcher => launcher.installed && !launcher.onPath);
  if (missingOnPath.length > 0) {
    findings.push({ severity: "warning", code: "launcher_not_on_path", message: `설치된 Claude 계정별 바로가기 ${missingOnPath.map(l => l.command).join(", ")}를 터미널에서 찾을 수 없습니다.`, action: "~/.frogprogsy/bin을 PATH 뒤에 한 번만 추가한 뒤 새 터미널을 여세요. 평범한 `claude` 명령은 원래 Claude Code로 유지됩니다." });
  }
