...
| `frogp models continuity [--json]` | none | Reads the running proxy's sorted model continuity report. Text groups required work by model, reports separate model and setting-location counts, and keeps inactive or past-session references in a diagnostic section. `--json` prints the `GET /api/model-continuity` document unchanged. |
...
| `frogp models continuity remove <reference-id>` | config and affected Claude Code model cache | Removes an optional setting, one exact saved fallback candidate, or a saved continuity policy. Removing one candidate preserves the policy's automatic mode and other candidates. Required provider defaults and past-session aliases cannot be removed. The current target from the live report guards the write. |

Automatic continuity is **off by default**. `retired` moves only ordinary routed requests whose primary model has retired, `transient` handles eligible temporary failures, and `all` enables both. Classifier, model-mixing, helper, and subagent references never use automatic continuity; change or remove their explicit setting instead.
