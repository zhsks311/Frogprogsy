...
自动模式判断、Model Mixing、网页搜索与图像处理模型、子任务模型都有固定职责，不能静默切换到行为不同的模型。每张卡片会显示准确的角色或列表序号，因此请使用**永久替换**；位置可选时也可使用**删除此设置**。Provider 默认模型不能删除，只能替换。删除 helper 会保留其他选项并关闭该 helper；删除列表中的最后一个模型仍会保留明确的空列表。模型显示状态与子任务模型优先顺序彼此独立；处理模型问题不会清除其他尚未保存的顺序编辑。已保存但不再可用的优先模型会标为不可用，但不会重新加入可选模型列表。
...
Activity 的 usage section 是 local accounting，不是 provider invoice view。FrogProgsy 会把每个已结束的 `/v1/messages` request 记录到 `~/.frogprogsy/usage.jsonl`，保留 provider 提供的 usage，并为未成功完成的 request 保留失败状态。没有提供 usage 的 provider request 会归入 `unreported`，不会显示为 0 token。
