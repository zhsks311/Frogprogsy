...
최신 자료에 연결할 수 없거나 검증을 통과하지 못해도 시작은 계속됩니다. 마지막으로 검증된 사본이 있으면 그 자료를 쓰고, 처음 설치해 저장된 사본이 없으면 설치 버전에 포함된 전체 모델 자료를 씁니다. 이 과정은 API 키, 사용자가 고른 기본 모델, 숨긴 모델, 직접 추가한 모델을 바꾸지 않습니다.
