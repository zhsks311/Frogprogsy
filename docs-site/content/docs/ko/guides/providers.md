...
FrogProgsy는 proxy가 요청을 받기 전, 시작할 때 한 번 검증된 최신 모델 자료를 확인합니다. 새 모델 목록을 적용하려면 proxy를 다시 시작하세요. 실행 중인 proxy에서 대시보드만 여는 것으로는 모델 자료가 갱신되지 않습니다.

최신 자료에 연결할 수 없거나 검증을 통과하지 못해도 시작은 계속됩니다. 마지막으로 검증해 저장한 사본과 설치 버전의 기본 자료를 비교해 catalog revision이 더 높은 쪽을 사용합니다. 저장된 사본이 없는 새 설치에서는 기본 자료를 씁니다. 이 과정은 API 키, 사용자가 고른 기본 모델, 숨긴 모델, 직접 추가한 모델을 바꾸지 않습니다.

다시 시작한 뒤 `frogp models`를 실행해 현재 자료 출처와 모델별 지원 상태를 확인하세요.
