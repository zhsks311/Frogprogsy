...
import { claudeLauncherBinDir, claudeLauncherFileName, claudeProfileShortcutName } from "../src/claude-launchers";
...
describe("Claude Code home management API", () => {
  test("GET reports installed only for the exact regular managed launcher for each profile", async () => {
    const root = mkdtempSync(join(tmpdir(), "frog-profile-installed-"));
    const frogHome = join(root, "frog");
    const previousFrogHome = process.env.FROGPROGSY_HOME;
    const cfg = config();
    cfg.claudeProfiles!.profiles = [
      { ...cfg.claudeProfiles!.profiles[0]!, claudeHome: join(root, ".claude-default") },
      { id: "cp_exact", name: "Exact", claudeHome: join(root, ".claude-exact") },
      { id: "cp_unmarked", name: "Unmarked", claudeHome: join(root, ".claude-unmarked") },
      { id: "cp_wrong", name: "Wrong", claudeHome: join(root, ".claude-wrong") },
      { id: "cp_symlink", name: "Symlink", claudeHome: join(root, ".claude-symlink") },
    ];
    for (const profile of cfg.claudeProfiles!.profiles) mkdirSync(profile.claudeHome, { recursive: true });
    process.env.FROGPROGSY_HOME = frogHome;
    const binDir = claudeLauncherBinDir();
    mkdirSync(binDir, { recursive: true });
    const launcherPath = (profileId: string) => {
      const profile = cfg.claudeProfiles!.profiles.find(item => item.id === profileId)!;
      return join(binDir, claudeLauncherFileName(claudeProfileShortcutName(profile)));
    };
    const managedScript = (profileId: string) => process.platform === "win32"
    ...
    writeFileSync(launcherPath("cp_exact"), managedScript("cp_exact"), "utf8");
    writeFileSync(launcherPath("cp_unmarked"), "user-owned launcher\n", "utf8");
    writeFileSync(launcherPath("cp_wrong"), managedScript("cp_someone_else"), "utf8");
    const symlinkDestination = join(root, "managed-symlink-destination");
    writeFileSync(symlinkDestination, managedScript("cp_symlink"), "utf8");
    symlinkSync(symlinkDestination, launcherPath("cp_symlink"), "file");
    ...
    try {
      ...
      const installedById = Object.fromEntries(
        body.profiles
          .filter((profile: { id: string }) => profile.id !== "cp_default")
          .map((profile: { id: string; shortcut: { installed: boolean } }) => [profile.id, profile.shortcut.installed]),
      );

      expect(res?.status).toBe(200);
      expect(installedById).toEqual({
        cp_exact: true,
        cp_unmarked: false,
        cp_wrong: false,
        cp_symlink: false,
  ...
  test("PATCH tells the user to install Claude Code when a rename cannot create its shortcut", async () => {
    const cfg = config();
    const res = await __requestLogTest.handleManagementAPI(
      new Request("http://localhost/api/claude-profiles/cp_work", {
        method: "PATCH",
      ...
      cfg,
      {
        ...
        syncClaudeLaunchers: () => ({
          ...
          launchers: [],
          warnings: ["install Claude Code and run frogp refresh"],
        }),
      },
    );
    ...
    expect(await json(res!)).toMatchObject({
      profile: { id: "cp_work", name: "Team" },
      warning: "The account rename was saved, but Claude Code is not installed. Install Claude Code and run frogp refresh to create its shortcut.",
      launcherSync: { success: true, realClaudeResolved: false },
    });
  });
  ...
  test("POST tells the user to install Claude Code when the saved account has no shortcut", async () => {
    const cfg = config();
    const homeRoot = mkdtempSync(join(tmpdir(), "frog-profile-api-missing-claude-"));
    try {
      const res = await __requestLogTest.handleManagementAPI(
        new Request("http://localhost/api/claude-profiles", {
          method: "POST",
      ...
      expect(await json(res!)).toMatchObject({
        profile: { name: "team", shortcut: { command: "claude-team", installed: false } },
        warning: "The account was saved, but Claude Code is not installed. Install Claude Code and run frogp refresh to create its shortcut.",
        launcherSync: { success: true, realClaudeResolved: false },
      });
    } finally {
      rmSync(homeRoot, { recursive: true, force: true });
